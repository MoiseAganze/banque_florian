...
class BankAccountSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Générez le numéro de compte unique
        account_number = self.generate_account_number()
        
        # Créez le compte bancaire en associant l'utilisateur connecté et en ajoutant les autres informations
        bank_account = BankAccount.objects.create(
            user=self.context['request'].user,  # Associe l'utilisateur connecté
            account_number=account_number,  # Génère un numéro de compte unique
